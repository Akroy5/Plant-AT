...
class ConvBNReLU(nn.Module):
    def __init__(self, input, output1, kernel_size=3, stride=1, groups=1, norm_layer=None):
        super(ConvBNReLU, self).__init__()
        padding = (kernel_size - 1) // 2
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self.conv = nn.Sequential(nn.Conv2d(input, output1, kernel_size, stride, padding, groups=groups, bias=False), norm_layer(output1), nn.ReLU6(inplace=True))
...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio, norm_layer=None):
        ...
        # Add a 1x1 convolution if the spatial dimensions of shortcut and conv do not match
        self.adjust_conv = None
        if not self.use_res_connect:
            self.adjust_conv = nn.Conv2d(inp, oup, 1, stride, 0, bias=False)

    def forward(self, x):
        #print(f"shape before inverted residual : {x.shape}")
        if self.use_res_connect:
            out = self.conv(x)
            #print(f"shape after stride 1 inverted residual : {out.shape}")
            return x + out
        else:
            shortcut = self.adjust_conv(x) if self.adjust_conv else x
            out = self.conv(x)
            #print(f"shape after stride 2 inverted residual : {out.shape}")
            return shortcut + out



#Single layer of Neighborhood attention Transformer for local inter dependencies in image input tensor
...
class FFN(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU,drop=0.0,):
        ...
        self.ffn1 = nn.Conv2d(in_features, hidden_features, kernel_size=1, stride=1 )
        self.act = act_layer()
        self.ffn2 = nn.Conv2d(hidden_features, out_features, kernel_size=1, stride=1)
    ...
    def forward(self, x):
           ...
           #print(f"shape before ffn : {x.shape}")
           x = self.ffn1(x)  # Apply self.ffn1 as a function to x
           #print(f"shape after ffn1 : {x.shape}")
           x = self.act(x)
           x = self.drop(x)
           #print(f"shape before ffn2 : {x.shape}")
           x = self.ffn2(x)  # Apply self.ffn2 as a function to x
           #print(f"shape after ffn2 : {x.shape}")
           x = self.drop(x)
           return x
...
class NATLayer(nn.Module):
    def __init__(self, att_dim, num_heads, ffn_dims, kernel_size=7, dilation=None, qkv_bias=True, qk_scale=None, drop=0.3, attn_drop=0.3, drop_path=0.3, act_layer=nn.GELU, norm_layer=nn.LayerNorm, layer_scale=None, num_iterations=2):
        super().__init__()
        self.att_dim = att_dim
        self.num_heads = num_heads
        self.FFN_ratio = ffn_dims
        self.num_iterations = num_iterations
        self.layers = nn.ModuleList([
            nn.ModuleDict({
                'norm1': norm_layer(att_dim),
                'attn': NeighborhoodAttention(att_dim, kernel_size=kernel_size, dilation=dilation, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop),
                'norm2': norm_layer(att_dim),
                'ffn': FFN(in_features=att_dim, hidden_features=ffn_dims, out_features=att_dim, act_layer=act_layer, drop=drop)
        ...
        if layer_scale is not None and type(layer_scale) in [int, float]:
            ...
            self.gamma1 = nn.Parameter(layer_scale * torch.ones(att_dim), requires_grad=True)
            self.gamma2 = nn.Parameter(layer_scale * torch.ones(att_dim), requires_grad=True)
        self.init_weights()
    ...
    def forward(self, x):
        for layer in self.layers:
            shortcut = x
            # Apply attention
            x = x.permute(0, 2, 3, 1)  # Correct permutation for NAT
            x = layer['attn'](layer['norm1'](x))
            x = x.permute(0, 3, 1, 2)  # Permutation back to original
            # Residual connection
            if self.layer_scale:
            ...
            x = x.permute(0, 2, 3, 1)  # Correct permutation for FFN
            x = layer['ffn'](layer['norm2'](x))
            x = x.permute(0, 1, 2, 3)  # Permutation back to original
            # Residual connection
            if self.layer_scale:
                x = shortcut + self.drop_path(self.gamma2 * x)
            else:
                x = shortcut + self.drop_path(x)
        return x

# Separable self attention transformer
...
class LinearSelfAttention(nn.Module):
    def __init__(self, embed_dim, attn_dropout=0):
        super().__init__()
        self.qkv_proj = Conv2D(embed_dim, 1 + 2 * embed_dim, kernel_size=1, bias=True, norm=False, act=False)
        self.attn_dropout = nn.Dropout(attn_dropout)
        self.out_proj = Conv2D(embed_dim, embed_dim, kernel_size=1, bias=True, norm=False, act=False)
        self.embed_dim = embed_dim

    def forward(self, x):
        # Project the input to q, k, and v
        qkv = self.qkv_proj(x)
        q, k, v = torch.split(qkv, [1, self.embed_dim, self.embed_dim], dim=1)
        
        # Compute context scores
        context_score = F.softmax(q, dim=-1) + 1e-6
...
#loop of linearattnffn

class LinearAttnFFN(nn.Module):
    def __init__(self, embed_dim, ffn_latent_dim, dropout=0.2, attn_dropout=0.2, num_iterations=2):
        super().__init__()
        self.num_iterations = num_iterations

        self.layers = nn.ModuleList([
            nn.ModuleDict({
                'pre_norm_attn': nn.Sequential(
                ...
                'pre_norm_ffn': nn.Sequential(
                    nn.GroupNorm(num_channels=embed_dim, eps=1e-5, affine=True, num_groups=1),
                    Conv2D(embed_dim, ffn_latent_dim, kernel_size=1, stride=1, bias=True, norm=False, act=True),
                    nn.Dropout(dropout),
                    Conv2D(ffn_latent_dim, embed_dim, kernel_size=1, stride=1, bias=True, norm=False, act=False),
                    nn.Dropout(dropout)
                )
            }) for _ in range(num_iterations)
        ])
    ...
    def forward(self, x):
        for layer in self.layers:
            # self attention
            x = x + layer['pre_norm_attn'](x)
            # Feed Forward network
            x = x + layer['pre_norm_ffn'](x)
        return x


class LGAttention(nn.Module):
    def __init__(self, attn_dim, oup, ffn_multiplier, num_heads, patch_size, num_iterations=3):
        super(LGAttention, self).__init__()

        self.patch_h, self.patch_w = patch_size
        ffn_dims = int((ffn_multiplier*attn_dim)//16*16)
        self.nat_layer = NATLayer(attn_dim, num_heads, ffn_dims, num_iterations=num_iterations)
        self.linear_attn_ffn = LinearAttnFFN(attn_dim, ffn_dims, num_iterations=num_iterations)
        self.conv_proj = nn.Sequential(
            nn.Conv2d(attn_dim, oup, kernel_size=1, stride=1, padding=0 ),
            nn.BatchNorm2d(oup),
            nn.SiLU()
        )
...
class RobustMultiScaleFusion(nn.Module):
    def __init__(self, in_channels):
        super(RobustMultiScaleFusion, self).__init__()
        self.in_channels = in_channels
        
        # Multi-scale convolutions
        self.conv1 = nn.Conv2d(in_channels, in_channels // 4, kernel_size=1)
        self.conv3 = nn.Conv2d(in_channels, in_channels // 4, kernel_size=3, padding=1)
        self.conv5 = nn.Conv2d(in_channels, in_channels // 4, kernel_size=5, padding=2)
        self.conv7 = nn.Conv2d(in_channels, in_channels // 4, kernel_size=7, padding=3)
    ...
    def forward(self, x1, x2):
        # Multi-scale processing for both inputs
        x1_multi = torch.cat([self.conv1(x1), self.conv3(x1), self.conv5(x1), self.conv7(x1)], dim=1)
        x2_multi = torch.cat([self.conv1(x2), self.conv3(x2), self.conv5(x2), self.conv7(x2)], dim=1)
        
        # Combine multi-scale features
        fused = self.fuse_conv(x1_multi + x2_multi)
        ...
        spatial_att = self.spatial_attention(torch.cat([torch.max(fused, 1)[0].unsqueeze(1), torch.mean(fused, 1).unsqueeze(1)], dim=1))
...
class PlantAttentionModel(nn.Module):
    def __init__(self, image_size, width_multiplier, num_plant_types, num_diseases, patch_size=(2, 2)):
        super().__init__()
        ih, iw = image_size
        ...
        self.conv_0 = ConvBNReLU(3, channels[0], kernel_size=3, stride=2)

        self.layer_1 = nn.Sequential(
            InvertedResidual(channels[0], channels[0], stride=1, expand_ratio=exp_mult)
        )

        self.layer_2 = nn.Sequential(
            InvertedResidual(channels[0], channels[1], stride=2, expand_ratio=exp_mult),
            LGAttention(attn_dim[0], channels[1], ffn_multiplier, 2, patch_size=patch_size),
        )
        self.linear_attn_2 = LinearAttnFFN(channels[1], 128)
        self.nat_2 = NATLayer(channels[1], 2, ffn_dims=128)
        self.fusion_2 = RobustMultiScaleFusion(channels[1])



        self.layer_3 = nn.Sequential(
            InvertedResidual(channels[1], channels[1], stride=1, expand_ratio=exp_mult),
            InvertedResidual(channels[1], channels[2], stride=2, expand_ratio=exp_mult),
            LGAttention(attn_dim[1], channels[2], ffn_multiplier, 2, patch_size=patch_size),
        )
        self.linear_attn_3 = LinearAttnFFN(channels[2], 256)
        self.nat_3 = NATLayer(channels[2], 2, ffn_dims=256)
        self.fusion_3 = RobustMultiScaleFusion(channels[2])
        self.layer_4 = nn.Sequential(
            InvertedResidual(channels[2], channels[2], stride=1, expand_ratio=exp_mult),
            InvertedResidual(channels[2], channels[3], stride=2, expand_ratio = exp_mult),
            LGAttention(attn_dim[2], channels[3], ffn_multiplier, 2, patch_size=patch_size),
        )
        self.linear_attn_4 = LinearAttnFFN(channels[3], 512)
        self.nat_4 = NATLayer(channels[3], 2, ffn_dims=512)
        self.fusion_4 = RobustMultiScaleFusion(channels[3])
        ...
        self.plant_head = nn.Linear(channels[1], num_plant_types)
        self.disease_head = nn.Linear(channels[1], num_diseases)
    ...
    def forward(self, x):
        ...
        x = self.layer_1(x)
        x = self.layer_2(x)
        x = self.fusion_2(self.nat_2(x), self.linear_attn_2(x))
        x = self.layer_3(x)
        x = self.fusion_3(self.nat_3(x), self.linear_attn_3(x))
        x = self.layer_4(x)
        x = self.fusion_4(self.nat_4(x), self.linear_attn_4(x))
        x = self.global_pool(x)
        x = x.view(x.size(0), -1)
        x = self.fc(x)
        plant_output = self.plant_head(x)
...
model = PlantAttentionModel(image_size=(IMAGE_SIZE, IMAGE_SIZE), width_multiplier=1, num_plant_types=len(plant_types), num_diseases=len(disease_names), patch_size=(2, 2))
...
for epoch in range(num_epochs):
   ...
   # val_disease_metrics = calculate_metrics(np.array(val_disease_targets), np.array(val_disease_predictions), np.array(val_disease_scores))
    ...
    checkpoint_directory = '/Akash/ENV/Checkpoint_Saver/weights_{epoch}.pth'
