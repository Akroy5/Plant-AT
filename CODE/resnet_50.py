...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, plant_types, disease_names):
        ...
        self.fc_plant = nn.Linear(512 * block.expansion, len(plant_types))
        self.fc_disease = nn.Linear(512 * block.expansion, len(disease_names))
    ...
    def forward(self, x):
        ...
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
        plant_output = self.fc_plant(x)
        disease_output = self.fc_disease(x)
        return plant_output, disease_output




def ResNet50(plant_types, disease_names):
    return ResNet(Bottleneck, [3, 4, 6, 3], plant_types, disease_names)
...
model = ResNet50(plant_types=plant_types, disease_names=disease_names)
