...
class ConvTokenizer(nn.Module):
    ...
    def forward(self, x):
        #print(f"Before convotokenizer: {x.shape}")
        x = self.proj(x).permute(0, 2, 3, 1)
        #print(f"After convotokenizer with permute : {x.shape}")
        if self.norm is not None:
            x = self.norm(x)
            #print(f"print after norm in convotokenizer: {x.shape}")
        return x
...
class ConvDownsampler(nn.Module):
    ...
    def forward(self, x):
        #print(f"print the shape before Convodownsampler : {x.shape}")
        x = self.reduction(x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
        #print(f"print the shape after Convodownsampler : {x.shape}")
        x = self.norm(x)
        #print(f"print the shape after norm in convodownsampler : {x.shape}")
        return x
...
class Mlp(nn.Module):
    ...
    def forward(self, x):
       # print(f"x before MLP layer :{x.shape}")
        x = self.fc1(x)
...
class NATLayer(nn.Module):
    ...
    def forward(self, x):
        #print(f"Before the NAT ATTention: {x.shape}")
        if not self.layer_scale:
            shortcut = x
            x = self.norm1(x)
            #print(f"After norm : {x.shape}")
            x = self.attn(x)
            #print(f"After Nat attn {x.shape}")
            x = shortcut + self.drop_path(x)
            #print(f"after shortcut : {x.shape}")
            x = x + self.drop_path(self.mlp(self.norm2(x)))
            #print(f"Final output of NAT attention: {x.shape}")
            return x
        shortcut = x
        x = self.norm1(x)
        #print(f"After norm1 : {x.shape}")
        x = self.attn(x)
        #print(f"After Nat attn 1 {x.shape}")
        x = shortcut + self.drop_path(self.gamma1 * x)
        #print(f"After Nat attn 1 with learnable parameter {x.shape}")
        x = x + self.drop_path(self.gamma2 * self.mlp(self.norm2(x)))
        #print(f"After final output of Nat attn 1 with learnable parameter gamma 2 {x.shape}")
...
class NATBlock(nn.Module):
    ...
    def forward(self, x):
        for i, blk in enumerate(self.blocks):
            #print("Level ", i)
            x = blk(x)
           # print(f"print shape after NAT Block: {x.shape}")
...
class NAT(nn.Module):
    def __init__(
        ...
        self.plant_head = nn.Linear(self.num_features, self.num_plant_types)
        self.disease_head = nn.Linear(self.num_features, self.num_diseases)
...
model = NAT(
    ...
    num_heads=[3, 6, 12, 24],
    mlp_ratio=4.,
    num_plant_types=len(train_dataset.plant_types),
    num_diseases=len(train_dataset.diseases),
...
# Set up TensorBoard
writer = SummaryWriter('runs/plant_disease_experiment1')
...
for epoch in range(num_epochs):
   ...
   # val_disease_metrics = calculate_metrics(np.array(val_disease_targets), np.array(val_disease_predictions), np.array(val_disease_scores))
    ...
    checkpoint_directory = '/Akash/ENV/Checkpoint_Saver/weights_nattiny.pth'
