...
class VGG(nn.Module):
    def __init__(
        self, features: nn.Module, num_plant_types: int, num_diseases: int, init_weights: bool = True, dropout: float = 0.5
    ) -> None:
        super().__init__()
        ...
        self.plant_classifier = nn.Linear(4096, num_plant_types)
        self.disease_classifier = nn.Linear(4096, num_diseases)
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        x = torch.flatten(x, 1)
        x = self.classifier(x)
        plant_output = self.plant_classifier(x)
...
def vgg19(*, num_plant_types: int, num_diseases: int, weights: Optional[Any] = None, progress: bool = True, **kwargs: Any) -> VGG:
    return _vgg("E", False, num_plant_types, num_diseases, weights, progress, **kwargs)

def _vgg(cfg: str, batch_norm: bool, num_plant_types: int, num_diseases: int, weights: Optional[Any], progress: bool, **kwargs: Any) -> VGG:
    if weights is not None:
        kwargs["init_weights"] = False
    model = VGG(make_layers(cfgs[cfg], batch_norm=batch_norm), num_plant_types, num_diseases, **kwargs)
    if weights is not None:
...
model = vgg19(num_plant_types=len(plant_types), num_diseases=len(disease_names))
...
for epoch in range(num_epochs):
   ...
   # val_disease_metrics = calculate_metrics(np.array(val_disease_targets), np.array(val_disease_predictions), np.array(val_disease_scores))
    ...
    checkpoint_directory = '/Akash/ENV/Checkpoint_Saver/VGG19weights.pth/'
